...
class logistic_bias(D1):
    ...
    def potential(self, x):
        # Define logistic transitions for the bias
        logistic_bias = (1 / (1 + np.exp(3 * (x + np.sqrt(4))))+ 1 / (1 + np.exp(-3 * (x - np.sqrt(4)))))


        # Add the double well base potential
        double_well = (x ** 2 - 1.2) ** 2

        return 10*logistic_bias + double_well

    def force_ana(self, x):
        # Derivative of the logistic_bias terms
        bias_force = (
                -10 * 3 * np.exp(3 * (x + np.sqrt(4))) / (1 + np.exp(3 * (x + np.sqrt(4)))) ** 2 +
                10 * 3 * np.exp(-3 * (x - np.sqrt(4))) / (1 + np.exp(-3 * (x - np.sqrt(4)))) ** 2
        )

        # Derivative of the double well potential
        double_well_force = -4 * (x ** 2 - 1.2) * x

        # Combine the derivatives to compute the force
        force = bias_force + double_well_force
...
class Metadynamics(D1):
    def __init__(self, param, interval):
        self.w = param[0]
        self.sigma = param[1]
        self.interval = interval  # Interval for adding biases
        self.times_added = []  # Store positions where Gaussians are added

    def add_bias(self, x):
        """Add a Gaussian bias centered at position x."""
        self.times_added.append(x)

    def biased_potential(self, x):
        """Compute the total potential (double-well + bias)."""
        base_potential = (x ** 2 - 1) ** 2
        bias_potential = sum(
            self.w * np.exp(-((x - xt) ** 2) / (2 * self.sigma ** 2))
            for xt in self.times_added
        )
        return base_potential + bias_potential

    def biased_force(self, x):
        """Compute the total force (from potential + bias)."""
        base_force = -4 * (x ** 2 - 1) * x
        bias_force = sum(
            -self.w * (x - xt) / (self.sigma ** 2) * np.exp(-((x - xt) ** 2) / (2 * self.sigma ** 2))
            for xt in self.times_added
        )
        return base_force + bias_force
